...
class Graph_Cut_Solver:
    ...
    # df(p) = a
    VAR_ALPHA = -1
    # df(p) occluded or not in the right image
    VAR_ABSENT = -2
    MAX_ITER = 4

    def __init__(self, left, right, 
    search_depth=50, max_steps=-1,occ_cost=-1,
    smooth_cost=-1, smooth_thresh=10):
        ...
        self.smooth_cost = smooth_cost if smooth_cost > 0 else  occ_cost * 0.2
        self.smooth_thresh = smooth_thresh
        self.energy = float('inf')

        # construct the network neighbours
        ...
        self.is_left_behind = np.abs(left_diff) < self.smooth_thresh
    ...
    def solve(self):
        ...
        for i in range(self.MAX_ITER):
            ...
            print(f'Label order in iteration {i}: {label_order}')
    ...
    def expand_label(self, label):
        ...
        self.calculate_E_data_occ(G, label)
        self.calculate_E_smooth(G, label)
        self.calculate_E_unique(G, label)
    ...
    def calculate_E_data_occ(self, G, label):
    ...
    def calculate_E_smooth(self, G, label):
    ...
    def calculate_E_unique(self, G, label):
        h, w = self.img_shape
        y_idx, x_idx = self.img_indices
    ...
    def add_unique_weights(self, G, sinks, sources):
        for i in range(sources.size):
            # really big number (maxsize seems to cause problems)
            G.add_edge(sources[i], sinks[i], int('864775807'), 0)
    
    def get_smooth_penalty(self, labels):
        p_idx, q_idx = self.neighbours
        if type(labels) is np.ndarray:
            labels = labels[self.is_left_behind]
        smoothness = np.full(p_idx.shape[1], self.smooth_cost, dtype=np.float)
        p_idx_shifted, is_p_in_img = self._shift(p_idx[:, self.is_left_behind], labels)
        q_idx_shifted, is_q_in_img = self._shift(q_idx[:, self.is_left_behind], labels)
        diff_right = self.right[list(p_idx_shifted)] - self.right[list(q_idx_shifted)]
        is_left_behind = self.is_left_behind.copy()
        is_left_behind[is_left_behind] = np.logical_not(is_p_in_img & is_q_in_img)
        smoothness[is_left_behind] = 0
        return smoothness
