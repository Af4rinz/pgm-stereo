...
class Graph_Cut_Solver:
    ...
    search_depth=50, max_steps=0, occ_cost=-1, 
    ...
    visualize=False):
        ...
        self.max_steps = max_steps if max_steps else search_depth
        self.search_interval = (search_depth // self.max_steps) + (search_depth % self.max_steps > 0)
        self.search_steps = np.arange(0, search_depth + 1, self.search_interval)[::-1] * -1
        ...
        np.indices = np.indices(self.img_shape)
        ...
        self.is_left_under = np.abs(diff_left) < smooth_thresh

    def solve(self):
        self.labels = np.full(self.img_shape, -1, dtype=np.int)
        label_done = np.zeros(len(self.search_steps), dtype=np.bool)

        for i in range(self.max_steps):
            if i==0:
                # random ordering to avoid weighted preference 
                label_order = np.random.permutation(len(self.search_steps))
    ...
    def add_E_data_occ(self, G, label):
        y_idx, x_idx = self.indices
        is_label = self.labels == label
        is_occluded = self.labels == -1
        ...
        active_ssd[is_label | is_occluded] =  (-1 - self.occ_cost)
        ...
        label_ssd[is_label | is_occluded] = (-1 - self.occ_cost)
        ...
        for nidx in range(self.neighbours.shape[2]):
            ...
            if p_label == q_label:
                if not is_p_active or not is_q_active:
                    continue
                self.add_smooth_wights(G, node_active_p, node_active_q, 0, penalty_p, penalty_q, 0)
                continue
    ...
    def get_smooth_penalty(self, labels):
        ...
        if type(labels) is np.ndarray:
            labels = labels[self.under]
        smoothness = np.full(p_idx.shape[1], self.smooth_cost_lo, dtype=np.float)
        p_idx_shifted, is_p_in_img = self.shift(p_idx[:, self.is_left_under], labels)
        q_idx_shifted, is_q_in_img = self.shift(q_idx[:, self.is_left_under], labels)
    ...
    def add_unique_weights(self, G, sinks, sources):
        for i in range(sources.size):
            G.add_edge(sources[i], sinks[i], sys.maxsize, 0)
